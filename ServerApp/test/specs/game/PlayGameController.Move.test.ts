import { HAND_TILE_ID_MILITARY, HAND_TILE_ID_SAMURAI, USER, USER_WITH_MOVE, afterAllPlay, afterEachPlay, beforeAllPlay, beforeEachPlay, createGame } from '.';
...
test(`Valid move order should work`, async () => {
    const game = await createGame(['PLAYER', 'PLAYER_WITH_MOVE'], 'PLAYER_WITH_MOVE');
...
test(`Trying to move hand tile from a free board tile should NOT work`, async () => {
    const game = await createGame(['PLAYER', 'PLAYER_WITH_MOVE'], 'PLAYER_WITH_MOVE');
